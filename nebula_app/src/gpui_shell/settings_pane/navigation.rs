...
pub(super) const SECTION_SEARCH_TERMS: [&str; 11] = [
    ...
    "profiles 配置文件 shell terminal 终端 completion 补全 startup 启动 ai message notifications system native windows toast alerts bell 提醒 通知 系统通知 windows通知 弹窗 消息 右下角 ai消息通知 ai 消息通知 ai消息弹窗 ai 消息弹窗 铃声 duration timeout persistent auto-dismiss 时长 秒 常驻 自动关闭",
