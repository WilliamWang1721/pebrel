...
fn sidebar_tabs_keep_square_hit_targets_and_files_precedes_git(cx: &mut TestAppContext) {
    ...
    for id in ["details-info", "side-panel-files", "side-panel-git"] {
        let bounds = cx.debug_bounds(id).unwrap();
        assert_eq!(bounds.size.width, px(36.0), "{id}: {bounds:?}");
        assert_eq!(bounds.size.height, px(36.0), "{id}: {bounds:?}");
    }
    ...
    assert_eq!(cx.debug_bounds("details-outline").unwrap().size.width, px(36.0));
    assert!(cx.debug_bounds("side-panel-files").unwrap().size.width > px(36.0));
